...
def vars_to_vcf(var_sites, chromosome, outfile): #to write the information to an outfile and get chromosome
    ...
    chromosome : path to the reference sequence file (str)
...
def vcf_file(fastq_file, reffile):
    """overarching function that calls in def fasta_file , def fastq_sequence , def variant_sites
    fastq_file : path to a .fq file (str)
...
vars_to_vcf(variable_sites, "reference", "result.vcf")#reffile bottom bit, "result.vcf"#becomes outfile in bottom bit
...
if __name__ == '__main__': #only need this for command line executable. Not relevant for ipython notebook usage
    try:
        ...
        sys.exit(1)
    variable_sites = vcf_file(fastq_file, reffile)
sys.exit(0)

#chmod u+x SNP_finder.py #make executable using chmod u+x. very last thing to do at the end of the script
